...
describe('ripple-openehr-jumper/lib/buildInverse', () => {
  let buildInverse;
  let buildJSONFile;
  let helpers;
  ...
  beforeAll(() => {
    ...
    jumperPath = path.join(__dirname, '../templates/contacts/');
  ...
  beforeEach(() => {
    ...
    mockery.registerMock('qewd-ripple/lib/jumper', {helpers: helpers});

    buildJSONFile = jasmine.createSpy().and.callFake(buildJSONFileFn);
    mockery.registerMock('./buildJsonFile', buildJSONFile);

    delete require.cache[require.resolve('../../lib/buildInverse')];
    buildInverse = require('../../lib/buildInverse');
  });
